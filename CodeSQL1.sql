...
CREATE TABLE CaLam (
    ...
    TgianBatDau TIME,
    TgianKetThuc TIME,
    HeSoLuong FLOAT CONSTRAINT CK_CaLam_HeSo CHECK (HeSoLuong >= 0),
    CONSTRAINT CK_CaLam_ThoiGian CHECK (TgianKetThuc > TgianBatDau)
);
...
CREATE TABLE TaiKhoan (
    ...
    IDVaiTro INT CONSTRAINT FK_TaiKhoan_VaiTro FOREIGN KEY REFERENCES VaiTro(ID) 
			ON UPDATE CASCADE 
			ON DELETE CASCADE,
    TenDangNhap VARCHAR(50) UNIQUE NOT NULL CONSTRAINT CK_TenDN CHECK (LEN(TenDangNhap) >= 5),
    MatKhau VARCHAR(255) NOT NULL CONSTRAINT CK_MK CHECK (LEN(MatKhau) >= 6),
...
CREATE TABLE NhanVien (
    ...
    ChucVu NVARCHAR(50),
    LuongCB DECIMAL(18,2) CONSTRAINT CK_NhanVien_Luong CHECK (LuongCB >= 0)
);
...
-- 3. XE & TÀI SẢN
CREATE TABLE Xe (
    BienSoXe VARCHAR(20) PRIMARY KEY CONSTRAINT CK_Xe_BSX CHECK (LEN(BienSoXe) >= 6),
...
CREATE TABLE ThietBi (
    ...
    NgayCaiDat DATE CHECK (NgayCaiDat <= GETDATE()),
    GiaLapDat DECIMAL(18,2) CHECK (GiaLapDat >= 0)
);
...
CREATE TABLE KhungGio (
    ...
    CONSTRAINT CK_KhungGio_ThoiGian CHECK (ThoiGianKetThuc > ThoiGianBatDau)
);
...
CREATE TABLE TheXeThang (
    ...
    NgayHetHan DATE,
    TrangThai BIT DEFAULT 1,
    CONSTRAINT CK_TheXeThang_Han CHECK (NgayHetHan >= NgayDangKy)
);
...
CREATE TABLE Voucher (
    ...
    HanSuDung DATE CHECK (HanSuDung >= CAST(GETDATE() AS DATE)),
...
CREATE TABLE DatCho (
	...
	CONSTRAINT CK_DatCho_TG CHECK (TgianKetThuc > TgianBatDau)
);
...
CREATE TABLE PhieuGiuXe (
	...
	CONSTRAINT CK_PhieuGiuXe_ThoiGian CHECK (TgianRa IS NULL OR TgianRa > TgianVao)
);
...
CREATE TABLE LichLamViec (
    ...
    NgayKetThuc DATE,
    TrangThai BIT DEFAULT 0,
    SoNgayDaLam INT DEFAULT 0 CHECK (SoNgayDaLam >= 0),
    CONSTRAINT CK_Lich_Ngay CHECK (NgayKetThuc >= NgayBatDau)
);
